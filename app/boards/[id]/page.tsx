  ...
  useEffect(() => {
    ...
    const mockPins: Pin[] = Array.from({ length: 12 }, (_, i) => {
      ...
      return {
        ...
        image_url: `/placeholder.svg?height=${height}&width=${width}&query=beautiful+${foundBoard?.title?.toLowerCase().replace(/\s+/g, "+")}`,
        alt_text: `Pin ${i + 1}`,
        width,
        height,
        user_id: "demo-user",
      ...
      <div className="min-h-screen bg-background">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground mb-6">The board you're looking for doesn't exist.</p>
            <Button asChild>
          ...
          <>
            ...
            <div className="text-center mb-8">
              <div className="flex items-center justify-center mb-4">
                <h1 className="text-4xl font-bold text-foreground">{board.title}</h1>
                {board.is_private && <Lock className="w-6 h-6 text-muted-foreground ml-3" />}
              </div>

              {board.description && (
                <p className="text-lg text-muted-foreground mb-4 max-w-2xl mx-auto">{board.description}</p>
              )}
            ...
            {/* Board pins */}
            {boardPins.length > 0 ? (
              <MasonryGrid pins={boardPins} onLoadMore={handleLoadMore} hasMore={false} loading={false} />
            ) : (
              <div className="text-center py-20">
                <div className="w-24 h-24 bg-muted rounded-full flex items-center justify-center mx-auto mb-4">
                  <Plus className="w-12 h-12 text-muted-foreground" />
                </div>
                <h3 className="text-xl font-semibold text-foreground mb-2">No pins yet</h3>
                <p className="text-muted-foreground mb-6">Start adding pins to this board</p>
                <Button className="bg-primary hover:bg-primary/90 text-primary-foreground">
                  <Plus className="w-4 h-4 mr-2" />
                  Add your first pin
                </Button>
