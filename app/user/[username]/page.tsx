...
import { MapPin, ExternalLink, Calendar, Settings, UserPlus, UserCheck, Share } from "lucide-react"
...
export default function UserProfilePage() {
  ...
  useEffect(() => {
    ...
    const filteredBoards = boards.filter((board) => board.user_id === (foundUser?.id || "current-user"))
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-center mb-8">
          ...
          {user.bio && <p className="text-muted-foreground mb-6 max-w-2xl mx-auto">{user.bio}</p>}
          ...
          <div className="flex items-center justify-center space-x-6 text-sm text-muted-foreground mb-6">
            ...
            {user.website_url && (
              <div className="flex items-center">
                ...
                <a href={user.website_url} target="_blank" rel="noopener noreferrer" className="hover:text-primary">
          ...
          <div className="flex justify-center space-x-8 mb-6">
            <div className="text-center">
              <p className="text-2xl font-bold text-foreground">{user.follower_count.toLocaleString()}</p>
            ...
            <div className="text-center">
              <p className="text-2xl font-bold text-foreground">{user.pin_count.toLocaleString()}</p>
          ...
          <div className="flex justify-center space-x-3">
            {isOwnProfile ? (
              <>
                <Button onClick={() => setShowEditProfile(true)} variant="outline" className="bg-transparent">
              ...
              <>
                <Button
                  ...
                  className={isFollowing ? "bg-transparent" : "bg-primary hover:bg-primary/90 text-primary-foreground"}
                >
                  {followLoading ? (
        ...
        <Tabs defaultValue="pins" className="w-full">
          ...
          <TabsContent value="pins">
            {userPins.length > 0 ? (
              <MasonryGrid pins={userPins} onLoadMore={handleLoadMore} hasMore={false} loading={false} />
            ) : (
              <div className="text-center py-20">
                ...
                <h3 className="text-xl font-semibold text-foreground mb-2">
                  {isOwnProfile ? "No pins yet" : `${user.display_name || user.username} hasn't pinned anything yet`}
                </h3>
                <p className="text-muted-foreground mb-6">
                  {isOwnProfile ? "Start creating pins to share your ideas" : "Check back later for new pins"}
                </p>
                {isOwnProfile && (
            ...
            {userBoards.length > 0 ? (
              ...
              <div className="text-center py-20">
                ...
                <h3 className="text-xl font-semibold text-foreground mb-2">
                  {isOwnProfile
                    ? "No boards yet"
                    : `${user.display_name || user.username} hasn't created any boards yet`}
                </h3>
                <p className="text-muted-foreground mb-6">
                  {isOwnProfile ? "Create boards to organize your pins" : "Check back later for new boards"}
                </p>
                {isOwnProfile && (
                  <Button asChild className="bg-primary hover:bg-primary/90 text-primary-foreground">
      ...
      {isOwnProfile && <EditProfileModal user={user} open={showEditProfile} onOpenChange={setShowEditProfile} />}
    </div>
  )
}
